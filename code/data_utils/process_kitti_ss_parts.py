...
tot_im = len(im_list)
step_im = int(tot_im/parts)
print 'tot im',tot_im
print 'step im',step_im
for part in range(1,parts+1):
    xbeg = int((part-1)*step_im)
    xend = int(part*step_im) 
    print part,xbeg,xend
    hkl.dump(X[xbeg:xend], os.path.join(home, 'X_' + split + suffix+ str(part)+'.hkl'))
    hkl.dump(source_list[xbeg:xend], os.path.join(home, 'sources_' + split + suffix + str(part)+ '.hkl'))
